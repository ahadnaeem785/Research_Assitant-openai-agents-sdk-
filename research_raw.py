...
# # State dictionary
# class ResearchFlowState(dict):
#     def __init__(self, **kwargs):
#         super().__init__(**kwargs)
#         self.setdefault("query", "")
#         self.setdefault("r_query", "")
#         self.setdefault("limit", 5)
#         self.setdefault("semantic_scholar_results", [])
#         self.setdefault("pubmed_results", [])
#         self.setdefault("arxiv_results", [])
#         self.setdefault("insights", [])
#         self.setdefault("consensus_meter", None)
#         self.setdefault("short_summary", "")
#         self.setdefault("conclusion", "")
...
#             "retmode": "json",
#             "retmax": limit,
#             # "api_key": ncbi_api_key  # Uncomment if using
#         }
...
# # Agents
# refine_agent = Agent(
#     name="RefineAgent",  # Replace with your API key
#     instructions="Refine the user's query to be concise, specific, and clear. Return only the refined query text.",
#     model="gpt-4o-mini"
# )

# semantic_scholar_agent = Agent(
#     name="SemanticScholarAgent",
#     instructions="Use the provided query and limit to fetch relevant research papers from Semantic Scholar.",
#     model="gpt-4o-mini"
# )

# pubmed_agent = Agent(
#     name="PubMedAgent",
#     instructions="Use the provided query and limit to fetch relevant research papers from PubMed.",
#     model="gpt-4o-mini"
# )

# arxiv_agent = Agent(
#     name="ArxivAgent",
#     instructions="Use the provided query and limit to fetch relevant research papers from ArXiv.",
#     model="gpt-4o-mini"
# )
...
#         "For each insight, provide 2-3 sentences explaining the finding in depth, ensuring clarity and relevance. "
#         "Include specific details from the papers, such as methodologies, results, or implications."
#     ),
#     model="gpt-4o-mini",
...
#     name="ReportAgent",
#     instructions=(
#         "Generate a formatted Markdown report with the following sections: "
#         "Query, Short Summary, Consensus Meter, Insights, Conclusion, and Sources. "
#         "For Insights, list each insight with its title, detailed explanation, and URL as a clickable link. "
#         "For Sources, list all unique URLs from the insights as a bullet-point list."
#     ),
#     model="gpt-4o-mini"
# )

# async def research_workflow(state: ResearchFlowState):
#     # Step 1: Refine the query
#     refine_result = await Runner.run(refine_agent, state["query"])
#     state["r_query"] = refine_result.final_output.strip()
#     print(f"Refined Query: {state['r_query']}")

#     # Step 2: Parallel search with three agents
#     async def run_search(agent, query, limit):
...
#     semantic_task = run_search(semantic_scholar_agent, state["r_query"], state["limit"])
#     pubmed_task = run_search(pubmed_agent, state["r_query"], state["limit"])
#     arxiv_task = run_search(arxiv_agent, state["r_query"], state["limit"])
...
#     state["semantic_scholar_results"] = semantic_result
#     state["pubmed_results"] = pubmed_result
#     state["arxiv_results"] = arxiv_result

#     # Log results for debugging
#     print(f"Semantic Scholar Results: {len(state['semantic_scholar_results'])} papers")
#     print(f"PubMed Results: {len(state['pubmed_results'])} papers")
#     print(f"ArXiv Results: {len(state['arxiv_results'])} papers")
...
#     else:
#         instruction_prompt = (
#             f"You are an expert assistant tasked with analyzing research papers on the topic: '{state['r_query']}'.\n\n"
#             "Instructions:\n"
#             "1. Query Specificity: If the query is general (e.g., broad or ambiguous), set consensus_meter to null. If specific, estimate agreement in percentages.\n"
#             "2. Summary: Provide a concise overview in short_summary (3-4 sentences), starting with a topic definition, followed by key findings.\n"
#             "3. Consensus Meter: For specific queries, provide percentages (Yes, Possibly, No) based on paper agreement.\n"
#             "4. Key Insights: Generate 5-7 insights, each with:\n"
#             "   - Title: A clear, descriptive title.\n"
#             "   - Insight: 2-3 detailed sentences explaining the finding, including specific details like methodology, results, or implications.\n"
#             "   - URL: The exact URL from the paper.\n"
#             "5. Conclusion: Summarize the overall stance and implications in 2-3 sentences.\n\n"
#             "Output Format:\n"
#             "```json\n"
#             "{\n"
...
#         summarize_result = await Runner.run(summarize_agent, instruction_prompt)
#         summary_data = summarize_result.final_output
#         state.update({
#             "insights": summary_data.insights,
#             "consensus_meter": summary_data.consensus_meter,
...
#     )
#     sources_str = "\n".join(
#         f"- {insight.url}" for insight in state["insights"]
#     )
#     report_input = (
#         f"Query: {state['query']}\n"
#         f"Short Summary: {state['short_summary']}\n"
#         f"Consensus Meter: {state['consensus_meter']}\n"
#         f"Insights:\n{insights_str}\n"
#         f"Conclusion: {state['conclusion']}\n"
#         f"Sources:\n{sources_str}\n"
#         "Generate a formatted Markdown report with sections for Query, Short Summary, Consensus Meter, Insights, Conclusion, and Sources. "
...
#     state["final_report"] = report_result.final_output
...
#         limit=5
#     )
#     final_state = await research_workflow(initial_state)
...
# async def research_workflow(query: str, limit: int = 5) -> str:
#     # Step 1: Refine query
#     refined_query = await refine_query(query)

#     # Step 2: Parallel searches
#     semantic_papers, pubmed_papers, arxiv_papers = await search_papers(refined_query, limit)

#     # Step 3: Summarize
#     all_papers = (semantic_papers + pubmed_papers + arxiv_papers)[:limit]
#     summary_data = await summarize_papers(refined_query, all_papers)

#     # Step 4: Generate report
#     report = await generate_report(query, summary_data)
#     return report
...
#     await cl.Message(content="Welcome! Enter a research query (e.g., **Self Attention in Transformers**) to generate a full report.").send()
...
#     # Step 2: Parallel searches
#     semantic_papers, pubmed_papers, arxiv_papers = await search_papers(refined_query, limit)
#     print(f"Semantic Scholar Results: {len(semantic_papers)} papers")
#     print(f"PubMed Results: {len(pubmed_papers)} papers")
#     print(f"ArXiv Results: {len(arxiv_papers)} papers")
...
# async def run():
#     query = "Self Attention in Transformers"
#     report = await research_workflow(query, limit=5)
#     print(report)
# if __name__ == "__main__":
#     asyncio.run(run())